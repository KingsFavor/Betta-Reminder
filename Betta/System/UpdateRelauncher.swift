...
///   returns to Betta after running `brew upgrade` in Terminal).
/// - **Sandbox-safe & harmless.** Launches a new instance via `NSWorkspace`; only
///   quits the current one if that succeeded. If the launch is blocked, nothing
///   happens and the user simply relaunches manually.
@MainActor
